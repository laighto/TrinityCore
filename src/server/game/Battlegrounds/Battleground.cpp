        ...
        if (winnerArenaTeam && loserArenaTeam && winnerArenaTeam != loserArenaTeam)
        {
            if (winner != WINNER_NONE)
            {
                ...
                for(BattlegroundScoreMap::const_iterator itr = PlayerScores.begin(); itr != PlayerScores.end(); ++itr)
                {
                    ...
                    sql_query << "INSERT INTO armory_game_chart VALUES ('" << gameID << "', '" << resultTeamID << "', '" << plr->GetGUID() << "', '" << changeType << "', '" << ratingChange  << "', '" << resultRating << "', '" << itr->second->DamageDone << "', '" << itr->second->Deaths << "', '" << itr->second->HealingDone << "', '" << itr->second->DamageTaken << "', '" << itr->second->HealingTaken << "', '" << itr->second->KillingBlows << "', '" << m_MapId << "', '" << m_StartTimearmory << "', '" << m_EndTimearmory << "')";
...
void Battleground::RemovePlayerAtLeave(uint64 guid, bool Transport, bool SendPacket)
{
    ...
    if (player)
    {
        ...
        if (lich_exists && player->GetGUID() == lichGUID)
        {
            lich_exists = false;
            player->DeMorph();
...
void Battleground::StartBattleground()
{
    SetStartTime(0);
    SetLastResurrectTime(0);
    m_StartTimearmory = uint64(time(NULL));
