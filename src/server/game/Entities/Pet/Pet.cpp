        ...
        default:
        {
            switch (GetEntry())
            {
                ...
                case 27829: // Ebon Gargoyle
                {
                    ...
                    if (Player * p_owner = GetOwner()->ToPlayer())
                    {
                            ...
                            switch (itr->first)
                            {
                                ...
                                case 49638:
                                {
                                    if (const SpellInfo *proto = sSpellMgr->GetSpellInfo(itr->first))
                                        AddPct(impurityMod, proto->Effects[0].CalcValue());
                                }
                                break;
                            }
                        }
                    ...
                    SetBonusDamage(int32(GetOwner()->GetTotalAttackPowerValue(BASE_ATTACK) * 0.5f * impurityMod));
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float(petlevel - (petlevel / 4)));
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float(petlevel + (petlevel / 4)));
                    break;
                }
