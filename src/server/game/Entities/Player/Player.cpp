...
void Player::SetVisibleItemSlot(uint8 slot, Item* pItem)
{
    if (pItem)
    {
        if (pItem->GetFakeDisplayEntry())
            SetUInt32Value(PLAYER_VISIBLE_ITEM_1_ENTRYID + (slot * 2), pItem->GetFakeDisplayEntry());
        else
...
void Player::RestoreBaseRune(uint8 index)
{
    ...
    /*aura 62459 - if remove after rune was converted to death rune gives crash if use death rune*/
...
void Player::CreateWowarmoryFeed(uint32 type, uint32 data, uint32 item_guid, uint32 item_quality) {
    ...
    */
    if (GetGUIDLow() == 0)
    {
        TC_LOG_ERROR("Wowarmory", "player is not initialized, unable to create log entry!");
        return;
    }
    if (type <= 0 || type > 3)
    {
        TC_LOG_ERROR("Wowarmory", "unknown feed type: %d, ignore.", type);
        return;
    }
    if (data == 0)
    {
        TC_LOG_ERROR("Wowarmory", "empty data (GUID: %u), ignore.", GetGUIDLow());
        return;
    }
    ...
    feed.counter = 0;
    feed.date = time(NULL);
    TC_LOG_DEBUG("Wowarmory", "create wowarmory feed (GUID: %u, type: %d, data: %u).", feed.guid, feed.type, feed.data);
