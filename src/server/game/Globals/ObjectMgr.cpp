...
{
    TC_LOG_ERROR("Fake Items", "Deleting non - existing transmogrification entries...");
    ...
    if (!result)
    {
        TC_LOG_ERROR("server.loading", ">> Loaded 0 fake items. DB table `fake_items` is empty.");
        return;
    }
    ...
    TC_LOG_ERROR("server.loading", ">> Loaded %u fake items.", _fakeItemsStore.size());
}
