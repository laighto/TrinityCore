    ...
    */
    switch (_charCreateCallback.GetStage())
    {
        ...
        case 3:
        {
            ...
            sLog->outPlayerLog(LOG_FILTER_PLAYER, "Account: %d (IP: %s) Create Character:[%s] (GUID: %u)", GetAccountId(), IP_str.c_str(), createInfo->Name.c_str(), newChar.GetGUIDLow());
...
void WorldSession::HandleCharDeleteOpcode(WorldPacket & recv_data)
{
    ...
    sLog->outPlayerLog(LOG_FILTER_PLAYER, "Account: %d (IP: %s) Delete Character:[%s] (GUID: %u)", GetAccountId(), IP_str.c_str(), name.c_str(), GUID_LOPART(guid));
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
    ...
    std::string IP_str = GetRemoteAddress();
    sLog->outPlayerLog(LOG_FILTER_PLAYER, "Account: %d (IP: %s) Login Character:[%s] (GUID: %u) Level: %d",
...
void WorldSession::HandleChangePlayerNameOpcodeCallBack(PreparedQueryResult result, std::string newName)
{
    ...
    sLog->outPlayerLog(LOG_FILTER_PLAYER, "Account: %d (IP: %s) Character:[%s] (guid:%u) Changed name to: %s", GetAccountId(), GetRemoteAddress().c_str(), oldName.c_str(), guidLow, newName.c_str());
...
void WorldSession::HandleCharCustomize(WorldPacket& recv_data)
{
    ...
    if (result)
    {
        ...
        sLog->outPlayerLog(LOG_FILTER_PLAYER, "Account: %d (IP: %s), Character[%s] (guid:%u) Customized to: %s", GetAccountId(), GetRemoteAddress().c_str(), oldname.c_str(), GUID_LOPART(guid), newName.c_str());
    }
