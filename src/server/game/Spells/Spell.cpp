...
void Spell::TakeRunePower(bool didHit)
{
    ...
    if (SpellRuneCostEntry const* runeCostData = sSpellRuneCostStore.LookupEntry(m_spellInfo->RuneCostID))
    {
        ...
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_DISPEL:
            {
                ...
                for (Unit::AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
                {
                    ...
                    SpellInfo const *spells = sSpellMgr->GetSpellInfo(aura->GetId());
                    ...
                    if ((1<<aura->GetSpellInfo()->Dispel) & dispelMask)
                    {
                        ...
                        if (spells->IsPositive() == target->IsFriendlyTo(m_caster))
                            continue;
