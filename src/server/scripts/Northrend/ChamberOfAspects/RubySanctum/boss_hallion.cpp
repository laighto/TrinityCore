        ...
        struct boss_halionAI : public BossAI
        {
            ...
            void MeteorStrike()
            {
                ...
                if (pTarget)
                {
                    ...
                    me->SummonCreature(NPC_METEOR_STRIKE_NORTH, x, y, z, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 300000);
                }
            }
                    ...
                    while (uint32 eventId = events.ExecuteEvent())
                    {
                        switch (eventId)
                        {
                            ...
                            case EVENT_METEOR_STRIKE:
                                MeteorStrike();
                                events.ScheduleEvent(EVENT_METEOR_STRIKE, RAID_MODE(20000,20000,20000,35000));
                                break;
...
class spell_halion_portal : public SpellScriptLoader
{
    public:
        ...
        class spell_halion_portal_SpellScript : public SpellScript
        {
            ...
            void Register()
            {
                OnEffectHit += SpellEffectFn(spell_halion_portal_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_APPLY_AURA);
            }
        };
