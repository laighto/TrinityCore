...
enum GenericSpells
{
    ...
    SPELL_STURDUST      = 73619,
};

enum QuestCredits
{
    JC_QUEST_STURDUST_APPLIED = 39237
};

class spell_sturdust_no_2 : public SpellScriptLoader
{
   public:
       spell_sturdust_no_2() : SpellScriptLoader("spell_sturdust_no_2") { }

       class spell_sturdust_no_2_SpellScript : public SpellScript
       {
           PrepareSpellScript(spell_sturdust_no_2_SpellScript);

           bool Validate(SpellInfo const* /*spellInfo*/) OVERRIDE
           {
               if (!sSpellMgr->GetSpellInfo(SPELL_STARDUST_NO_2))
                   return false;
               return true;
           }
           ...
           void HandleScript()
           {
               ...
               {
                   if (Aura* aurEff = target->GetAura(SPELL_STURDUST))
                       return;
                   else
                   {
                       target->CastSpell(target, SPELL_STURDUST, false);
                       caster->ToPlayer()->KilledMonsterCredit(JC_QUEST_STURDUST_APPLIED, 0);
                   }
               }
               else if (Unit* target = GetHitPlayer())
               {
                   if (Aura* aurEff = target->GetAura(SPELL_STURDUST))
                       return;
                   else
           ...
           {
               OnCast += SpellCastFn(spell_sturdust_no_2_SpellScript::HandleScript);
           }
       ...
       {
           return new spell_sturdust_no_2_SpellScript();
       }
};
