...
void ChatLog::_Punish(Player* player, std::string& msg)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_CHATLOG_LEXIS_ACTION_CUSTOM))
    {
        m_LexicsCustomAction = LexicsCustomActions(sWorld->getIntConfig(CONFIG_CHATLOG_LEXIS_ACTION_CUSTOM_MODE));
        uint32 CastomSpell = sWorld->getIntConfig(CONFIG_CHATLOG_LEXIS_ACTION_CUSTOM_SPELL);
